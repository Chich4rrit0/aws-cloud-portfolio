...
$baselineAlarms = @($alarms.MetricAlarms | Where-Object { $_.AlarmName -in $expectedAlarms })
if ($baselineAlarms.Count -ne $expectedAlarms.Count) { throw 'One or more expected P5 alarms are missing.' }
if (@($baselineAlarms | Where-Object { @($_.AlarmActions).Count -gt 0 -or @($_.OKActions).Count -gt 0 -or @($_.InsufficientDataActions).Count -gt 0 }).Count -ne 0) { throw 'A P5 alarm unexpectedly has an automated action.' }
